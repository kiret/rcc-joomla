...
class EasyDiscussViewPost extends EasyDiscussView
{
	...
	public function resolve( $id = null )
	{
		...
		// Update the state of the item to 'is-resolved'
	...
	public function ajaxSubmitReply()
	{
		...
		{
			// @rule: Badges
			DiscussHelper::getHelper( 'History' )->log( 'easydiscuss.new.reply' , $table->user_id , JText::sprintf( 'COM_EASYDISCUSS_BADGES_HISTORY_NEW_REPLY', $question->title) );
		...
		$result['content']	= DiscussHelper::parseContent( $table->content );
		...
		$html	= ( $table->published == DISCUSS_ID_PENDING ) ? $tpl->fetch( 'reply.item.moderation.php' ) : $tpl->fetch( 'post.reply.item.php' );
		...
		$emailData['commentAuthor']	= ($my->id) ? $creator->getName() : JText::_('COM_EASYDISCUSS_GUEST');
		...
		$emailData['replyAuthor' ]	= ($my->id) ? $creator->getName() : JText::_('COM_EASYDISCUSS_GUEST');
		...
		if( ($config->get('main_sitesubscription') ||  $config->get('main_postsubscription') ) && $config->get('notify_subscriber') && $table->published == DISCUSS_ID_PUBLISHED)
		{
			...
			DiscussHelper::getHelper( 'Mailer' )->notifyThreadSubscribers( $emailData, array($posterEmail) );
		}
	...
	public function saveReply()
	{
		...
		if( $includePolls && $config->get( 'main_polls') )
		{
			...
			if( $pollItems )
			{
				...
				foreach( $pollItems as $item )
				{
					...
					if( !$poll->loadByValue( $value , $postTable->id , $multiplePolls ) )
					{
		...
		$postTable->content		= DiscussHelper::parseContent( $postTable->content );
