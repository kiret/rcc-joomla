...
class EasyDiscussViewIndex extends EasyDiscussView
{
	...
	public function filter()
	{
		...
		$posts		= $postModel->getData( false , $sort , null , $filterType , $categoryId, null, false);
		$posts		= DiscussHelper::formatPost($posts);
	...
	public function sort( $sort = null, $filter = null, $categoryId = null )
	{
		...
		if( $config->get('layout_featuredpost_style') != '0')
		{
			...
			{
				$ajax->script( 'discussQuery("#dc_featured-list").hide();');
			}
			else
			{
				$ajax->script( 'discussQuery("#dc_featured-list").show();');
			}
		}
