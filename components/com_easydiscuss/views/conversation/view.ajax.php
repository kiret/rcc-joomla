...
class EasyDiscussViewConversation extends EasyDiscussView
{
	...
	public function reply()
	{
		...
		$reply->authorName		= $reply->creator->getName();
		$reply->authorLink		= $reply->creator->getLink();
		$reply->authorAvatar	= $reply->creator->getAvatar();
		$reply->className		= $reply->creator->id == $my->id ? 'by-me' : 'by-user';
	 ...
	 * @access	public
	 */
	public function save( $content , $recipientId )
	{
		$config 	= DiscussHelper::getConfig();
		...
		$options			= new stdClass();
		$options->content	= JText::_( 'Your message was successfully sent.' );
		$options->title		= JText::_( 'COM_EASYDISCUSS_DIALOG_TITLE_NEW_CONVERSATION' );
