...
class DiscussMailerHelper
{
	...
	public static function notifySubscribers( $data, $excludes = array() )
	{
		...
		$subscribers = self::_getSubscribers( 'site', 0, 0 );
	...
	{
		$subscribers = self::_getSubscribers( 'post', $data['post_id'], $data['cat_id'] );
	 ...
	 */
	private static function _getSubscribers( $type, $cid, $categoryId, $params = array() )
	{
		$db		= DiscussHelper::getDbo();
	...
	protected static function _prepareBody( $data )
	{
		...
		if( $type == 'html' )
		{
			$template	= str_ireplace( '.php' , '.html.php' , $data['emailTemplate'] );
		}
