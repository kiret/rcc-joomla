...
class DiscussHelper
{
	...
	public static function permalinkSlug( $string )
	{
		...
		else
		{
			...
			// hällö wörldß became hallo-world instead haelloe-woerld thus above line is commented
	...
	public static function getVersion()
	{
		...
		{
			$version 	= $parser->xpath( 'easydiscuss/version' );
		...
		$element	= $parser->document->getElementByPath( 'easydiscuss/version' );
...
/*]]>*/";
			...
			$document->addScript( DISCUSS_MEDIA_URI  . '/' . $folder . '/' . 'abstract.js' );
	...
	public static function formatPost($posts, $isSearch = false , $isFrontpage = false )
	{
		...
		if(! empty($posts) > 0)
		{
			...
			for($i = 0; $i < count($posts); $i++)
			{
				...
				{
					// clean it to 1 liner
					$row->introtext	= preg_replace( '/\s+/', ' ', strip_tags(self::parseContent($row->content)) );
	...
	public static function formatComments( $comments )
	{
		...
		if( !$comments || !$config->get( 'main_comment' ) )
		{
			return false;
		}
	...
	public static function formatReplies( $result , $category = null )
	{
		...
		for($i = 0; $i < count($result); $i++)
		{
			...
			if( $config->get( 'layout_reply_editor' ) == 'bbcode' )
			{
				...
				$reply->content	= Parser::html2bbcode( $reply->content );

				// Parse bbcodes.
				$reply->content	= self::parseContent( $reply->content );
			}
	...
	public static function populateCategories($parentId, $userId, $outType, $eleName, $default = false, $isWrite = false, $isPublishedOnly = false, $showPrivateCat = true , $disableContainers = false )
	{
		$catModel 	= DiscussHelper::getModel( 'Categories' );
		...
		$html	= '';
		$html	.= '<select name="' . $eleName . '" id="' . $eleName .'" class="inputbox full-width">';
		if(! $isWrite)
	...
	public static function getBoardStatistics()
	{
		...
		$unansweredPosts	= $postModel->getUnresolvedCount();
		...
		$theme->set( 'unansweredPosts', $unansweredPosts );
	...
	public static function parseContent( $content )
	{
		...
		if( $config->get( 'layout_editor') == 'bbcode' )
		{
			...
			// if( floatval(phpversion() < '5.3') || self::getJoomlaVersion() < '1.6' )
			// {
				// Escape strings.
				$content	= DiscussHelper::getHelper( 'String' )->escape( $content );

				// Pass it to bbcode parser.
				$content	= Parser::bbcode( $content );

				$content	= nl2br($content);

				//Remove BR in pre tag
				$content = preg_replace_callback('/<pre.*?\>(.*?)<\/pre>/ims', array( 'Parser' , 'removeBr' ) , $content );
				$content = preg_replace_callback('/<ol.*?\>(.*?)<\/ol>/ims', array( 'Parser' , 'removeBr' ) , $content );
				$content = preg_replace_callback('/<ul.*?\>(.*?)<\/ul>/ims', array( 'Parser' , 'removeBr' ) , $content );
			...
			// } else {
			// 	$decoda = self::getBBCodeParser();

			// 	$decoda->defaults();

			// 	// Fixing compatibility ol
			// 	$content	= preg_replace( array('/\[list\=(.*?)\](.*?)\[\/list\]/ims') , array('[olist]\2[/olist]') , $content );
			// 	// Fixing compatibility with [*]
			// 	$content	= preg_replace( array('/\[\*\]\s?(.*?)\n/ims', '/\[\*\]\s?(.*?)/ims') , array('[li]\1[/li]', '[li]\1[/li]') , $content );

			// 	$decoda->reset($content);
			// 	$content = $decoda->parse();

			// 	// Fixing excessive spacing
			// 	$content = preg_replace_callback('/<ol.*?\>(.*?)<\/ol>/ims', array( 'Parser' , 'removeBr' ) , $content );
			// 	$content = preg_replace_callback('/<ul.*?\>(.*?)<\/ul>/ims', array( 'Parser' , 'removeBr' ) , $content );
			// 	$content = str_ireplace("</ul><br>", '</ul>', $content);
			// 	$content = str_ireplace("</ol><br>", '</ol>', $content);
