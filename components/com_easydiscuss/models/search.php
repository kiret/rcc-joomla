...
class EasyDiscussModelSearch extends EasyDiscussModel
{
	...
	*/
	...
	private function _buildQuery($sort = 'latest', $filter = '' , $category = '')
	{
		...
		$pquery .= ' a.`id`, a.`title`, a.`content`, a.`user_id`, a.`category_id`, a.`parent_id`, a.`user_type`, a.`created` AS `created`,';
		...
		$rquery .= ' a.`id`, a.`title`, a.`content`, a.`user_id`, a.`category_id`, a.`parent_id`, a.`user_type`,a.`created` AS `created`,';
		...
		$cquery .= ' a.`id`, a.`title`, a.`description` as `content`, a.`created_by` as `user_id`, a.`id` as `category_id`, 0 as `parent_id`, 0 AS `user_type`, a.`created` AS `created`,';
	...
	private function _buildQueryWhere( $type, $tbl )
	{
		$mainframe	= JFactory::getApplication();
		$db			= DiscussHelper::getDBO();
