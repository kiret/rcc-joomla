...
class EasyDiscussModelPosts extends EasyDiscussModel
{
	...
	*/
	 ...
	 * @return string
	 */
	private function _buildQuery( $sort = 'latest', $filter = '' , $category = '', $featuredOnly = 'all' , $reply = false , $exclude = array() )
	{
		$my				= JFactory::getUser();
		$config			= DiscussHelper::getConfig();
		...
		$filteractive	= (empty($filter)) ? JRequest::getString('filter', 'allposts') : $filter;
		$where			= $this->_buildQueryWhere( $filter , $category, $featuredOnly , $exclude );
		$db				= DiscussHelper::getDBO();
	...
	private function _buildQueryWhere($filter='' , $category = '', $featuredOnly = 'all' , $exclude = array() )
	{
		$mainframe		= JFactory::getApplication();
		$db				= DiscussHelper::getDBO();
		...
		{
			$my	= JFactory::getUser();
			$where[] = ' f.`created_by` = ' . $db->quote( $my->id );
		}
		...
		if( !empty( $category ) )
		{
			...
			$categoryWhere 	= ' a.`category_id` IN(';
			...
			for( $i = 0 ; $i < count( $category ); $i++ )
			{
				...
				{
					$categoryWhere	.= $db->Quote( $categoryId );
				}
	...
	public function getDiscussions( $options = array() )
	{
		...
		$query			= $this->_buildQuery( $sort , $filter , $category , $featured , false, $exclude );
	 ...
	 * @return array
	 */
	public function getData( $usePagination = true, $sort = 'latest' , $limitstart = null, $filter = '' , $category = '', $limit = null, $featuredOnly = 'all' )
	{
	...
	public function getPostsBy( $type, $typeId = 0, $sort = 'latest', $limitstart = null , $published = DISCUSS_FILTER_PUBLISHED , $search = '' , $limit = null )
	{
		...
		{
			case 'latest':
				$queryOrder	= ' ORDER BY a.`created` ASC';
				break;
			case 'popular':
	...
	public function getUnresolvedCount( $filter = '' , $category = '' , $tagId = '', $featuredOnly = 'all', $queryOnly = false )
	{
		...
		$query	= 'SELECT COUNT(a.`id`), a.`category_id` FROM ' . $db->nameQuote( '#__discuss_posts' ) . ' AS a';
