	...
	<div class="discuss-item<?php echo $post->islock ? ' is-locked' : '';?><?php echo $post->isresolve ? ' is-resolved' : '';?><?php echo $post->isFeatured ? ' is-featured' : '';?> <?php echo $readCss . $isRecent; ?>">
		...
		<div class="discuss-item-right">
			<div class="discuss-story">
				...
				<div class="discuss-statistic pull-right hide">
					<div class="status">
						<div class="item-count"><?php //echo $post->totalreplies;?> <?php echo $replies = !empty( $post->reply ) ? $post->totalreplies : 0; ?></div>
					...
					<div class="ph-10">
						...
						<div class="fs-11">
							...
							<span class="discuss-user-name mv- fs-11">
								by: <a href="<?php echo $post->user->getLink();?>" class="" title="<?php echo $this->escape( $post->user->getName() );?>"><?php echo $post->user->getName();?></a>
							</span>
						</div>
				...
				<div class="pull-left">
					<div class="discuss-last-replied pull-left mb-10">
						<?php if( isset( $post->reply ) ){ ?>
							...
							<?php if( $post->reply->id ){ ?>
							<a href="<?php echo DiscussRouter::getUserRoute( $post->reply->id );?>" class="ml-5 ">
								<?php if( $system->config->get( 'layout_avatar' ) ) { ?>
								<img src="<?php echo $post->reply->getAvatar();?>" alt="<?php echo $this->escape( $post->reply->getName() );?>" />
								<?php } ?>
								<span><?php echo $post->reply->getName();?></span>
							</a>
							<?php } else { ?>
								<?php echo $post->reply->poster_name; ?>
							<?php } ?>
