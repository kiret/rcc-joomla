...
class DiscussLessc {
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new DiscussLessc_parser($this, __METHOD__);
	...
	protected function makeParser($name) {
		$parser = new DiscussLessc_parser($this, $name);
	...
	protected function newFormatter() {
		$className = "Discusslessc_formatter_lessjs";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = "Discusslessc_formatter_$this->formatterName";
		}
...
class DiscussLessc_parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('DiscussLessc', 'preg_quote'),
					array_keys(self::$precedence))).')';

			$commentSingle = DiscussLessc::preg_quote(self::$commentSingle);
			$commentMultiLeft = DiscussLessc::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = DiscussLessc::preg_quote(self::$commentMultiRight);
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("DiscussLessc", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = DiscussLessc::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = DiscussLessc::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("DiscussLessc", "preg_quote"), $stop);
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = DiscussLessc::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.DiscussLessc::preg_quote($what), $m, !$until)) return false;
...
class Discusslessc_formatter_classic {
	public $indentChar = "  ";
...
class Discusslessc_formatter_compressed extends Discusslessc_formatter_classic {
...
class Discusslessc_formatter_lessjs extends Discusslessc_formatter_classic {
	public $disableSingle = true;
