	...
	public function getAdvanceOption( $active = '' )
	{
		...
		if( $loadField == true )
		{
			...
			if( $active == 'text' || $active == 'area' )
			{
				...
				{
					$addButton = '';
					$html = '<label>'. JText::_( 'COM_EASYDISCUSS_CUSTOMFIELDS_PLACEHOLDER' ) .'</label><input type="text" class="input-full" id="'.$fieldId.'" name="'.$fieldName.'" value="'.$param.'" />';
				}
			}
			else
			{
				...
				foreach( $params as $count => $param )
				{
					$html 	.= '<li class="remove'.$fieldType.'_'.$count.'">'
							...
							. '<input type="text" class="input-full '.$fieldType.'" id="'.$fieldType.'_'.$count.'" name="'.$fieldName.'" value="'.$param.'" />'
