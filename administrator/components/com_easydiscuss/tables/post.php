...
class DiscussPost extends JTable
{
	...
	{
		$this::$_pollsQuestion[ $this->id ] = $obj;
	}
		...
		if( count( $ids ) > 0 )
		{
			...
			if( count( $result ) > 0 )
			{
				foreach( $result as $item)
				{
					...
					$this::$_pollsQuestion[ $item->post_id ] = $poll;
				}//end foreach
			...
			foreach( $ids as $id )
			{
				if(! isset( $this::$_pollsQuestion[ $id ] ) )
				{
					$this::$_pollsQuestion[ $id ] = false;
				}
			}
	...
	public function setCommentsBatch( $ids, $limit = null, $limitstart = null )
	{
		if( count( $ids ) > 0 )
		{
			...
			$comments		= $postModel->getComments( $ids, $limit, $limitstart );
		...
		if( count( $ids ) > 0 )
		{
			...
			if( count( $result ) > 0 )
			{
				foreach( $result as $item)
				{
					...
					$this::$_attachments[ $item->uid ][] = $table;
				}//end foreach
	 ...
	 */
	public function bindPolls( $isNew = true , $polls = array() , $removePolls = array() , $multiplePolls = false , $pollQuestion = '' )
	{
		$config 		= DiscussHelper::getConfig();
		...
		foreach( $polls as $item )
		{
			...
			if( !$poll->loadByValue( $value , $this->id ) )
			{
	...
	public function getOwner()
	{
		...
		if( !isset( $owners[ $this->user_id ] ) )
		{
			...
			if( $this->user_id >= 0 )
			{
				...
				$owner->guest		= false;
				$owner->signature	= $user->getSignature();
			}
	...
	{
		if( !isset( $this::$_pollsQuestion[ $this->id ] ) )
		{
			$poll 	= DiscussHelper::getTable( 'PollQuestion' );
			...
			$this::$_pollsQuestion[ $this->id ] = $poll;
		}

		return $this::$_pollsQuestion[ $this->id ];
	}
