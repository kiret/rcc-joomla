...
class EasyDiscussInstaller
{
	...
	private function checkDiscussion()
	{
		...
		if( !$this->db->loadResult() )
		{
			...
			$query		= 'INSERT IGNORE INTO `#__discuss_posts` ( `id`, `title`, `alias`, `created`, `modified`, `content`, `published`, `featured`, `isresolve`, `user_id`, `parent_id`, `user_type`) '
						. 'VALUES ( "1", "Thank you for choosing EasyDiscuss", "thank-you-for-choosing-easydiscuss", now(), now(), ' . $this->db->Quote($content['congratulation']) . ', 1, 1, 1,' . $this->db->Quote($suAdmin) .', 0, "member" ), '
						. '( "2", "Congratulations! You have successfully installed EasyDiscuss", "congratulations-succesfully-installed-easydiscuss", now(), now(), ' . $this->db->Quote($content['thankyou']) . ', 1, 0, 1,' . $this->db->Quote($suAdmin) .', 0, "member" ) ';
			$this->db->setQuery( $query );
			if( !$this->db->query() )
			{
	...
	private function checkACL()
	{
		...
		// Update acl rules
		$query	= "INSERT INTO `#__discuss_acl` (`id`, `action`, `default`, `description`, `published`, `ordering`, `public`) VALUES
					(1, 'add_reply', 1, 'Determines whether the user is allowed to post a new reply on a discussion.', 1, 0, 1),
					(2, 'add_question', 1, 'Determines whether the user is allowed to post a new discussion.', 1, 0, 1),
					(3, 'add_attachment', 1, 'Determines whether the user is allowed to upload attachments.', 1, 0, 1),
					(4, 'add_tag', 1, 'Determines whether the user is allowed to insert tags into their discussions.', 1, 0, 1),
					(5, 'edit_reply', 0, 'Determines whether the user is allowed to edit replies.', 1, 0, 0),
					(6, 'delete_reply', 0, 'Determines whether the user is allowed to delete replies.', 1, 0, 0),
					(7, 'mark_answered', 0, 'Determines whether the user is allowed to select an answer for the discussion.', 1, 0, 0),
					(8, 'lock_discussion', 0, 'Determines whether the user is allowed to lock or unlock a discussion.', 1, 0, 0),
					(9, 'edit_question', 0, 'Determines whether the user is allowed to edit an existing discussion.', 1, 0, 0),
					(10, 'delete_question', 0, 'Determines whether the user is allowed to delete an existing discussion.', 1, 0, 0),
					(11, 'delete_attachment', 0, 'Allows user to remove a file attachment from the reply or questions.', 1, 0, 0),
					(12, 'add_comment', 0, 'Determines whether the user is allowed to comment on existing replies.', 1, 0, 1),
	...
	private function getDiscussId()
	{
		...
		else
		{
			$query	= 'SELECT ' . $this->db->nameQuote( 'id' ) . ' '
				...
				. 'WHERE `option`=' . $this->db->Quote( 'com_easydiscuss' ) . ' '
...
class EasyDiscussDatabaseUpdate
{
	...
	public function update()
	{
		...
		if( !$this->isColumnExists('#__discuss_posts', 'islock') )
		{
		...
		if( !$this->db->loadResult() )
		{
			$query = "INSERT INTO `#__discuss_rules`(`id`, `command`, `title`, `description`, `callback`, `created`, `published`) VALUES ('14', 'easydiscuss.remove.reply', 'Remove a reply', 'This rule allows you to assign a badge for a user when they remove a reply.', '', NOW(), 1);";
			$this->db->setQuery($query);
			if( !$this->db->query() ) return false;
		}
