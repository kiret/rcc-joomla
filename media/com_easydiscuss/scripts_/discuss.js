...
EasyDiscuss.module('discuss', function($) {
  ...
  var discuss = window.discuss = {
			...
			var eventHandler = function()  {
				...
				switch (result.type)
				{
					...
					case 'success':
						...
						controller.addItem(result.html);
	...
	conversation: {
		...
		send: function()
		{
			...
			if (!contents)
			{
				console.log($('#conversationEmptyMessage'));
  ...
  disjax = window.disjax = disjax = {
	...
	_showPopup: function(options) {
		...
		if ($.isArray(options.buttons))
		{
			...
			dialog.find('.modal-footer').html('').show();
...
.done(function($) {
	...
	// Apply bootstrap's popover
	$('[rel=ed-popover]').popover({container: 'body', delay: { show: 100, hide: 100},animation: false, trigger: 'hover'});
    });
...
.done(function($) {
	...
	$('.discuss-searchbar').responsive({at: 650, switchTo: 'narrow'});
    });
